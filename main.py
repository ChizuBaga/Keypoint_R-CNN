...
def extract_keypoint(img, model, device):
    ...
    # Extract keypoints and their individual joint scores for the best detection
...
def prediction(model, data):
    ...
    if hasattr(model, 'predict'):
        ...
        # Check if SVC was trained with probability=True
...
class PostureApp:
    ...
    def load_selected_model(self, event=None):
        ...
        with self.data_lock:
            # --- NEW: Reset metrics when switching models ---
            self.inference_count = 0
            self.total_latency = 0.0
            self.total_confidence = 0.0
            self.last_conf = 0.0
    ...
    def rcnn_worker(self):
        while self.running:
            ...
            if frame is not None:
                ...
                # 1. Resize to half resolution to force faster inference
                small_w, small_h = 640, 360 
                small_frame = cv2.resize(frame, (small_w, small_h))
                img = cv2.cvtColor(small_frame, cv2.COLOR_BGR2RGB)
                
                # 2. Run the bottleneck model on the smaller image
                kp = extract_keypoint(img, self.detector, device)
                
                # 3. Scale the keypoint coordinates back to the original HD frame size
                if kp is not None:
                    scale_x = orig_w / small_w
                    scale_y = orig_h / small_h
                    kp[:, 0] *= scale_x  # Multiply X coordinates
                    kp[:, 1] *= scale_y  # Multiply Y coordinates
                ...
                time.sleep(0.05) # Wait for camera to warm up
    ...
    def inference_worker(self):
        while self.running:
            ...
            with self.data_lock:
                ...
                is_fresh = self.is_new_kp  # <--- NEW: Check if the data is fresh

            # NEW: Add 'and is_fresh' to this if-statement
    ...
    def refresh_ui_loop(self):
        ...
        if frame_to_show is not None:
            ...
            if lbl in ("Good", "Bad"):
                ...
                cv2.putText(frame_to_show, f"{lbl}  {conf*100:.0f}%",
        ...
        # --- NEW: Update Benchmarking Labels ---
